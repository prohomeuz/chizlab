...
export default function CategoryView({ categoryName, categorySlug, materials }) {
  ...
  return (
    <main className="min-h-screen bg-bg">
      <CategoryNav search={search} onSearchChange={setSearch} activeSlug={categorySlug} />
      {/* Sarlavha mockupda ko'rinmaydi — SEO/a11y uchun yashirin qoladi */}
      <h1 className="sr-only">{categoryName}</h1>

      <section className="px-10 pb-16 bp-lg:px-8 bp-md:px-6 bp-sm:px-5 bp-xs:px-4 bp-xs:pb-10">
        {/* Markazlashgan {filtr} chiplari — mockup tartibida */}
        <div className="mb-8 flex flex-wrap items-center justify-center gap-x-9 gap-y-1 bp-lg:gap-x-7 bp-md:gap-x-6 bp-sm:gap-x-5 bp-xs:gap-x-4 bp-xs:mb-6">
        ...
        {materials.length === 0 ? (
          ...
          <p className="text-center font-sf text-[18px] text-primary/60 bp-sm:text-[16px] bp-xs:text-[15px]">
            Tanlangan filtrlar bo&apos;yicha material topilmadi.
          </p>
        ) : (
          <div
            className="grid grid-cols-4 gap-5
              bp-lg:grid-cols-3 bp-md:grid-cols-3 bp-md:gap-4
              bp-sm:grid-cols-2 bp-sm:gap-4 bp-xs:grid-cols-2 bp-xs:gap-3"
          >
